...
model_young = joblib.load("artifacts/model_young.joblib")  # Model for age <= 25
model_rest = joblib.load("artifacts/model_rest.joblib")    # Model for age > 25
scaler_young = joblib.load("artifacts/scaler_young.joblib")
scaler_rest = joblib.load("artifacts/scaler_rest.joblib")

# calculate normalized risk for provided input medical history in form by end user
def calculate_normalized_risk(medical_history):
    ...
    # Normalize the total risk score between 0 to 1
...
def preprocess_input(input_dict):
    ...
    # Initialize DataFrame with zeros. This dataframe will be having 1 line only with index value 0
    df = pd.DataFrame(0, columns=expected_columns, index=[0])  
    # df.fillna(0, inplace=True)

     # Manually assign values for each categorical input based on input_dict
    # For each category (Gender, Region, Marital Status, BMI, Smoking, Employment), update the appropriate column to 1.
    # For numerical values like Age, Income, Dependants, Genetical Risk, assign them directly.
...
# Apply scaling (standardization) to relevant columns
def handle_scaling(age, df):
    ...
    # Extract scaler and list of columns to scale.
    # Note, "scaler" and "cols_to_scale" were dump using joblib 
    # Here, cols_to_scale = ['age','number_of_dependants','income_level', 'income_lakhs', 'insurance_plan', 'genetical_risk']
    # scaler is the object 
    cols_to_scale = scaler_object['cols_to_scale']
    scaler = scaler_object['scaler']

    df['income_level'] = None # Add dummy column income_level to satisfy scaler's expected input.
